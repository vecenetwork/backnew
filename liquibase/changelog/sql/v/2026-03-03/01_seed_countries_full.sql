-- Seed all 246 countries when table exists with partial/wrong data
-- Handles case: only USA (id=1) exists, need full list with USA at id=230

-- Step 1: Temporarily rename existing USA row to free the name for insert
UPDATE countries SET name = '__temp_usa_migration__' WHERE id = 1 AND name = 'United States of America';

-- Step 2: Insert all countries (ON CONFLICT skips existing id=1)
INSERT INTO countries (id, code, name, full_name, iso3, number, continent_code, display_order) VALUES
...
ON CONFLICT (id) DO UPDATE SET code=EXCLUDED.code, name=EXCLUDED.name, full_name=EXCLUDED.full_name, iso3=EXCLUDED.iso3, number=EXCLUDED.number, continent_code=EXCLUDED.continent_code, display_order=EXCLUDED.display_order;

-- Step 3: Point users from old USA (id=1) to correct USA (id=230)
UPDATE users SET country_id = 230 WHERE country_id = 1;
